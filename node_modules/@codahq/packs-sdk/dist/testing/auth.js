...
function setupAuth(manifestDir, packDef, opts = {}) {
    const auth = (0, helpers_2.getPackAuth)(packDef);
    if (!auth) {
        return (0, helpers_4.printAndExit)(`This Pack has no declared authentication. ` +
    ...
    switch (auth.type) {
        case types_1.AuthenticationType.None:
            return (0, helpers_4.printAndExit)(`This Pack declares AuthenticationType.None and so does not require authentication. ` +
        ...
        case types_1.AuthenticationType.Various:
            return (0, helpers_4.printAndExit)('This authentication type is not yet implemented');
        default:
...
class CredentialHandler {
    ...
    checkForExistingCredential() {
        ...
        if (existingCredentials) {
            const input = (0, helpers_5.promptForInput)(`Credentials already exist for this Pack, overwrite? (y/N): `, { yesOrNo: true });
    ...
    handleToken() {
        ...
        const input = (0, helpers_5.promptForInput)(`Paste the token or API key to use for this Pack:\n`, { mask: true });
        this.storeCredential({ endpointUrl, token: input });
        (0, helpers_3.print)('Credentials updated!');
    }
    handleMultiToken(headers) {
        if (headers.length === 0) {
            (0, helpers_4.printAndExit)(`Please define one or more entries for "headers" in the setUserAuthentication or setSystemAuthentication section of this Pack definition.`);
        }
        this.checkForExistingCredential();
        const endpointUrl = this.maybePromptForEndpointUrl();
        const credentials = { endpointUrl, headers: {} };
        for (const { name } of headers) {
            const value = (0, helpers_5.promptForInput)(`Enter the token to use for the "${name}" header for this Pack:\n`, { mask: true });
    ...
    handleWebBasic() {
        ...
        const username = (0, helpers_5.promptForInput)(`Enter the ${usernamePlaceholder} for this Pack:\n`);
        let password;
        if (!usernameOnly) {
            password = (0, helpers_5.promptForInput)(`Enter the ${passwordPlaceholder} for this Pack:\n`, { mask: true });
        }
    ...
    handleCustom(paramDefs) {
        ...
        if (paramDefs.length === 0) {
            (0, helpers_4.printAndExit)(`Please define one or more entries for "params" in the setUserAuthentication or setSystemAuthentication section of this Pack definition.`);
        }
        this.checkForExistingCredential();
        const endpointUrl = this.maybePromptForEndpointUrl();
        ...
        for (const param of parameters) {
            ...
            credentials.params[name] = (0, helpers_5.promptForInput)(`Enter the value to use for the '${name}'${descriptionText} parameter for this Pack:\n`, {
                mask: true,
            });
        }
        this.storeCredential(credentials);
    ...
    handleQueryParam(paramName) {
        if (!paramName) {
            (0, helpers_4.printAndExit)(`Please provide a paramName attribute in the setUserAuthentication or setSystemAuthentication section of this Pack definition.`);
        }
        this.checkForExistingCredential();
        const endpointUrl = this.maybePromptForEndpointUrl();
        const input = (0, helpers_5.promptForInput)(`Enter the token to use for the "${paramName}" url param for this Pack:\n`, {
            mask: true,
        });
    ...
    handleMultiQueryParams(paramDefs) {
        ...
        for (const paramDef of paramDefs) {
            const paramValue = (0, helpers_5.promptForInput)(`Enter the token to use for the "${paramDef.name}" url param for this Pack:\n`, { mask: true });
    ...
    _promptOAuth2ClientIdAndSecret(existingCredentials) {
        ...
        const newClientId = (0, helpers_5.promptForInput)(clientIdPrompt);
        ...
        const newClientSecret = (0, helpers_5.promptForInput)(clientSecretPrompt, { mask: true });
    ...
    handleOAuth2() {
        ...
        (0, helpers_3.print)(`*** Your application must have ${(0, oauth_server_2.makeRedirectUrl)(this._oauthServerPort)} whitelisted as an OAuth redirect url ` +
        ...
        this.storeCredential(credentials);
        (0, helpers_3.print)('Credential secrets updated! Launching OAuth handshake in browser...\n');
        ...
        (0, oauth_server_1.launchOAuthServerFlow)({
            ...
            afterTokenExchange: ({ accessToken, refreshToken, expires }) => {
                ...
                this.storeCredential(credentials);
                (0, helpers_3.print)('Access token saved! Shutting down OAuth server and exiting...');
            },
            scopes: requestedScopes,
        });
    }
    handleOAuth2ClientCredentials() {
        ...
        (0, oauth_server_3.performOAuthClientCredentialsServerFlow)({
            ...
            this.storeCredential(credentials);
            (0, helpers_3.print)('Access token saved!');
        }).catch(err => {
            throw err;
        });
    }
    handleAWSAccessKey() {
        ...
        const existingCredentials = this.checkForExistingCredential();
        const endpointUrl = this.maybePromptForEndpointUrl();
        const newAccessKeyId = (0, helpers_5.promptForInput)(`Enter the AWS Access Key Id for this Pack:\n`);
        const newSecretAccessKey = (0, helpers_5.promptForInput)(`Enter the AWS Secret Access Key for this Pack:\n`, { mask: true });
    ...
    handleAWSAssumeRole() {
        ...
        const existingCredentials = this.checkForExistingCredential();
        const endpointUrl = this.maybePromptForEndpointUrl();
        const newRoleArn = (0, helpers_5.promptForInput)(`Enter the AWS Role ARN for this Pack:\n`);
        const externalId = (0, helpers_5.promptForInput)(`[Optional] Enter the External ID for this Pack:\n`, { mask: true });
    ...
    maybePromptForEndpointUrl() {
        ...
        return (0, helpers_5.promptForInput)(`Enter the endpoint url for this Pack (for example, ${placeholder}):\n`);
    }
...
function readCredentialsFile(manifestDir) {
    const filename = path.join(manifestDir, CREDENTIALS_FILE_NAME);
    const fileContents = (0, helpers_6.readJSONFile)(filename);
...
function writeCredentialsFile(credentialsFile, credentials) {
    ...
    (0, helpers_7.writeJSONFile)(credentialsFile, fileContents, 0o600);
}
